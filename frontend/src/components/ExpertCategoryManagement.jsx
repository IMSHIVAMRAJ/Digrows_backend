...
const ExpertCategoryManagement = () => {
  const [categories, setCategories] = useState([]);
  const [newCategory, setNewCategory] = useState({ name: '', type: 'expert' });
  ...
  const fetchCategories = async () => {
    try {
      ...
      // ✅ FIX IS HERE: Filter the array to only show 'expert' categories
      const expertCategories = response.data.filter(cat => cat.type === 'expert');
      setCategories(expertCategories);
  ...
  const handleAddCategory = async () => {
    if (newCategory.name.trim()) {
      try {
        ...
        setNewCategory({ name: '', type: 'expert' });
        fetchCategories();
      } catch (error) {
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-black mb-2">Expert Category Management</h1>
        <p className="text-gray-600">Manage expert categories and specializations</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#f95e01] bg-white"
            >
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-black">Expert Categories ({categories.length})</h3>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Experts Count</th>
...
export default ExpertCategoryManagement;